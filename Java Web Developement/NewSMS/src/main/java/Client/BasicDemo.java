...
public class BasicDemo {
	public static void main(String[] args) throws Exception {
		...
		List<Student> templist = studentlist.stream().filter(x -> x.getAddress().equalsIgnoreCase("bengaluru")
		...
		Student student_lowestid = studentlist.stream().min((x, y) -> x.getSid().compareTo(y.getSid())).get();
		System.out.println(student_lowestid);
		boolean result = studentlist.stream().allMatch(x -> x.getAddress().equalsIgnoreCase("bengaluru"));
